...
class Backbone(nn.Module):
    def __init__(self, transition_channels, block_channels, n, phi, pretrained=False):
        super().__init__()
        #-----------------------------------------------#
        #   The input picture is 640, 640, 3
        #-----------------------------------------------#
        ids = {
            'l' : [-1, -3, -5, -6],
            'x' : [-1, -3, -5, -7, -8], 
    ...
    def forward(self, x):
        ...
        x = self.dark2(x)
        #----------------------------------------------------------------------#
        #   dark3 has an output of 80, 80, 512, which is a valid feature layer
        #----------------------------------------------------------------------#
        x = self.dark3(x)
        feat1 = x
        #------------------------------------------------------------------------#
        #   dark4 has an output of 40, 40, 1024, which is a valid feature layer
        #------------------------------------------------------------------------#
        x = self.dark4(x)
        feat2 = x
        #------------------------------------------------------------------------#
        #   dark5 has an output of 20, 20, 1024, which is a valid feature layer
        #------------------------------------------------------------------------#
        x = self.dark5(x)
        feat3 = x
